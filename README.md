...
| **内网穿透** | 内置 frpc，多条 TCP / HTTPS 隧道，可公开 MBrain 或手机其他服务，支持证书和 TLS 插件 |
...
**远程客户端**可在“设置 → 远程访问”添加自建 frps 隧道。支持多个隧道同时启用，各自启停和重试，网关停止时全部断开。公网 HTTPS 入口需在服务器配置，具体见[远程访问](docs/remote-access.md)。
...
需要从公网连接时，可进入 **设置 → 内网穿透**，配置自己的 frps。支持 TCP、HTTPS 透传、`https2http` 和 `tls2raw`，自定义服务隧道独立于 MCP 网关运行。证书导入、服务器示例与运行规则见[内网穿透指南](docs/remote-access.md)。
...
先确认首页显示“运行中”，并使用凭据页中的 Token。电脑连接时还需确认实际端口的 ADB 转发成功。网关仅监听手机回环地址；远程访问通过已配置的 frp 隧道和服务器 HTTPS 入口连接。
