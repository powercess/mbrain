// Modified by MBrain: full JSON Schema and raw JSON execution for MCP proxy tools.
...
interface McpTool {
    ...
    /** MBrain: complete schemas are preserved for proxied tools, including nested types. */
        ...
        putJsonObject("properties") {
            parameters.forEach { parameter ->
                putJsonObject(parameter.name) {
                    ...
                    if (parameter.type == ParameterType.ARRAY) put("items", buildJsonObject {})
                }
            }
        }
        putJsonArray("required") { parameters.filter { it.required }.forEach { add(it.name) } }
    }

    /** MBrain: raw JSON entry preserves explicit nulls for upstream MCP servers. */
    suspend fun executeJson(params: JsonObject): ToolResult = execute(
        params.entries.mapNotNull { (key, value) -> value.toToolValue()?.let { key to it } }.toMap()
    )
...
private fun JsonElement.toToolValue(): Any? = when (this) {
    JsonNull -> null
    is JsonObject -> entries.associate { it.key to it.value.toToolValue() }
    is JsonArray -> map { it.toToolValue() }
