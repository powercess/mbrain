// Modified by MBrain: atomic catalog replacement and raw JSON proxy execution.
...
class ToolRegistry {
    ...
    @Volatile private var tools: Map<String, McpTool> = emptyMap()

    @Synchronized
    fun replaceAll(next: List<McpTool>) {
        require(next.map { it.name }.distinct().size == next.size) { "Duplicate tool names" }
        tools = next.associateBy { it.name }
    }
    ...
    suspend fun executeToolJson(name: String, params: kotlinx.serialization.json.JsonObject): ToolResult {
        if (name in disabled) return ToolResult.error("tool_disabled", name)
        val tool = tools[name] ?: return ToolResult.error("Unknown tool: $name")
        return try { tool.executeJson(params) } catch (e: Exception) {
            if (e is kotlinx.coroutines.CancellationException) throw e
            ToolResult.error("Tool '$name' failed: ${e.message}")
        }
    }
