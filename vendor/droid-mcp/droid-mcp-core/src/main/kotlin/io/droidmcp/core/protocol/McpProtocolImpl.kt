// Modified by MBrain: preserve full proxy schemas, explicit null arguments and MCP result blocks.
...
class McpProtocolImpl(
    ...
    private fun handleToolsList(id: JsonElement?): String {
        val tools = visibleTools().map { tool ->
            buildJsonObject {
                ...
                tool.outputSchema?.let { put("outputSchema", it) }
    ...
    private suspend fun handleToolsCall(
        ...
        val toolResult = registry.executeToolJson(toolName, params["arguments"]?.jsonObject ?: buildJsonObject {})
        val durationMs = (System.nanoTime() - startedAt) / 1_000_000
        recordAudit(toolName, clientLabel, argumentsJson, toolResult, durationMs)

        toolResult.mcpResult?.let { return jsonRpcResponse(id, it) }
