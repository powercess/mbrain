// Modified by MBrain: bind HTTP/TLS to the configured host; skip unused device metadata.
...
class HttpTransport(
    ...
    val mdnsServiceName: String = run {
        val model = if (context != null) Build.MODEL else "local"
        val sanitized = model.replace(Regex("[^A-Za-z0-9-]"), "-")
