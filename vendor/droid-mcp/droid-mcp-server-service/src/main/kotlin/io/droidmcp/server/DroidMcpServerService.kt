// Modified by MBrain: notify the host when startup succeeds or fails.
...
abstract class DroidMcpServerService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (server == null) {
            try {
                ...
                onServerStarted(checkNotNull(server))
            } catch (e: Exception) {
    ...
    // MBrain lifecycle hooks: report the actual bind result to the host UI.
    protected open fun onServerStarted(server: DroidMcp) {}
