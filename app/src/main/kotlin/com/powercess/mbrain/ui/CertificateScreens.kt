...
internal fun CertificatesScreen(certificates: List<TunnelCertificate>, edit: (String) -> Unit, remove: (String) -> Unit) {
    ScreenList {
        item { OutlinedButton(onClick = { edit("new") }, modifier = Modifier.fillMaxWidth()) { Text("导入证书") } }
        if (certificates.isEmpty()) item { EmptyState(Icons.Outlined.VerifiedUser, "还没有证书", "导入 PEM 证书链；服务证书需同时导入对应私钥，CA 可单独导入。") }
        items(certificates, key = { it.id }) { certificate ->
            ...
            Group {
                ...
                GroupDivider()
                ActionRow(if (certificate.keyPem.isBlank()) "仅证书 / CA" else "包含私钥", "点按证书名称可替换", Icons.Outlined.Key,
...
internal fun CertificateEditor(initial: TunnelCertificate, dismiss: () -> Unit, save: (TunnelCertificate) -> Unit) {
    ...
    Surface(Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
        ...
        Scaffold(modifier = Modifier.fillMaxSize().safeDrawingPadding().imePadding(), contentWindowInsets = WindowInsets(0, 0, 0, 0),
            ...
            bottomBar = { Surface { Button(onClick = {
                scope.launch {
                    busy = true
                    try {
            ...
            }, enabled = !busy, modifier = Modifier.fillMaxWidth().padding(16.dp).heightIn(min = 50.dp)) { Text(if (busy) "处理中…" else "保存证书") } } },
            ...
            Box(Modifier.padding(padding).consumeWindowInsets(padding)) { ScreenList {
                ...
                item(key = "certificate") { OutlinedButton(onClick = { importingKey = false; picker.launch(arrayOf("*/*")) }, enabled = !busy, modifier = Modifier.fillMaxWidth()) {
                    Text(if (pem.isBlank()) "选择证书链文件" else "替换证书链文件")
                } }
                ...
                item(key = "key") { OutlinedButton(onClick = { importingKey = true; picker.launch(arrayOf("*/*")) }, enabled = !busy, modifier = Modifier.fillMaxWidth()) {
                    Text(if (keyPem.isBlank()) "选择私钥文件（CA 可省略）" else "替换私钥文件")
                } }
